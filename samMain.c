...
void WriteWav(FILE *file, char *buffer, int bufferlength) {
    if (file == NULL) return;
    //RIFF header
...
int samMain(int argc, char **argv) 
{
    ...
    if (wavfilename != NULL)
    {
        ...
        {
            file = fopen(wavfilename, "wb");
        }
        WriteWav (file, GetBuffer(), GetBufferLength() / 50);
    }
    else
    {
#ifdef USE_SDL 
        OutputSound();
#else
        PL_Play (1, 22050, 8, GetBuffer(), GetBufferLength()/50, TRUE);
#endif
    }
